...
topTerms[, 1:5] # first five topics
```

#### Use the topic model output to enrich the corpus with features

The final topic features distribution in the corpus is different than what is reported in the vignette. This is due to fundamental changes within the **`quanteda`** package regarding their tokenization. The sensitivity of the topical output is a good reminder about the importance of optimizing the preprocessing and the topic modelling itself (which is not dealt with in detail here).
