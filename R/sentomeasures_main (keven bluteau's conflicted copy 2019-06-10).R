...
sento_measures <- function(sentocorpus = NULL, lexicons, ctr) {
  ...
  aggregatedMeasures <- aggregate(sentiment, ctr)
  sentomeasures <- set_dimensions(aggregatedMeasures) #to be updated with language integration
...
aggregate_docs <- function(sentiment, by, how = get_hows()$docs, weightingParamDocs ) {
  ...
  names <- stringi::stri_split(colnames(sentomeasures$sentiment)[4:ncol(sentomeasures$sentiment)], regex = "--")
