...
#pre-process corpus
corpus = tm::tm_map(corpus, tm::content_transformer(tolower))  # Change capital letter to lower-case
...
#Note that there is multiple article per day. The next step is to merge per topic to obtain topic/daily sentiment measures. 
#The number appearing in each row of one of the topic column is the probability in percent that the document is from that topic.  
